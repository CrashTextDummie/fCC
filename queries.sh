...
echo "$($PSQL "select name from games left join teams on games.winner_id=teams.team_id where year = 2018 and round = 'Final' group by name")"

echo -e "\nList of teams who played in the 2014 'Eighth-Final' round:"
echo "$($PSQL "select name from games g inner join teams t on g.winner_id=t.team_id or g.opponent_id=t.team_id where year = 2014 and round = 'Eighth-Final' group by name")"
